...
-- block_type 枚举见 plugins/shield/block_type.go（1-10，数值稳定）。
CREATE TABLE IF NOT EXISTS {table} (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    time        DATETIME NOT NULL,           -- 拦截时刻（Go 侧传 time.Time）
    trace_id    TEXT NOT NULL DEFAULT '',    -- 链路 ID（仅 shield_event 内部追溯，不与 access_log 关联）
    block_type  INTEGER NOT NULL,            -- 拦截类别
    ...
    status_code INTEGER NOT NULL DEFAULT 0,  -- 拦截响应码（403/413/429）
