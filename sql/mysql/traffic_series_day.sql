...
-- 桶表达式照 shield_event_stats_daily.sql 现状（DATE_FORMAT 取日期，UTC 口径）。
SELECT bucket,
       SUM(CASE WHEN src = 'access' THEN 1 ELSE 0 END) AS ok_count,
       SUM(CASE WHEN src = 'shield' THEN 1 ELSE 0 END) AS blocked_count
FROM (
  SELECT DATE_FORMAT(time, '%Y-%m-%d') AS bucket, 'access' AS src
  FROM {table}
  WHERE time >= ? AND time <= ?
  UNION ALL
  SELECT DATE_FORMAT(time, '%Y-%m-%d') AS bucket, 'shield' AS src
  FROM {table2}
  WHERE time >= ? AND time <= ?
) t
GROUP BY bucket
