...
-- 输出列：bucket（UTC 小时桶标签 'YYYY-MM-DD HH:00:00'）、ok_count、blocked_count。
-- 口径说明：ok_count 与 summary.req_ok 同口径（含静态资源、含 4xx/5xx）；blocked_count = 拦截计数。
-- 时间边界 time >= from AND time <= to，各桶之和 = summary 总数（对账依据）。
-- 桶表达式照 shield_event_stats_daily.sql 日期范式把粒度改到小时（MySQL 存 UTC 墙上时间字面值）。
SELECT bucket,
       SUM(CASE WHEN src = 'access' THEN 1 ELSE 0 END) AS ok_count,
       SUM(CASE WHEN src = 'shield' THEN 1 ELSE 0 END) AS blocked_count
FROM (
  SELECT DATE_FORMAT(time, '%Y-%m-%d %H:00:00') AS bucket, 'access' AS src
  FROM {table}
  WHERE time >= ? AND time <= ?
  UNION ALL
  SELECT DATE_FORMAT(time, '%Y-%m-%d %H:00:00') AS bucket, 'shield' AS src
  FROM {table2}
  WHERE time >= ? AND time <= ?
) t
GROUP BY bucket
