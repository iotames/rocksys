...
--                path 先经 SUBSTRING_INDEX 截取 '?' 前的路径段，避免带查询串的路径漏匹配后缀。
--   uv         = COUNT(DISTINCT client_ip, COALESCE(user_agent,''))（IP+UA 多参去重口径，
--                ua 空串退化计 1）。
...
SELECT
  ...
  (SELECT COUNT(*) FROM {table} WHERE time >= ? AND time <= ?
     AND NOT (LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.js'
       OR LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.css'
       OR LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.map'
       OR LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.ico'
       OR LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.png'
       OR LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.jpg'
       OR LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.jpeg'
       OR LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.gif'
       OR LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.svg'
       OR LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.webp'
       OR LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.woff'
       OR LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.woff2'
       OR LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.ttf'
       OR LOWER(SUBSTRING_INDEX(path, '?', 1)) LIKE '%.eot')) AS req_pv,
