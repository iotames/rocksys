...
-- 定位：只做登记 + 状态汇总，不驱动任何任务。
-- kind 取值：configurable=可配型（关联 easyconf 开关）/ system=系统级只读（被改则重启按 name 重置）。
-- last_status 取值：success / failed / skipped / partial；仅回写者有意义（本期=geoip_sync）。
-- 域边界：本表是任务定义（工厂）的登记面，只记「上一轮调度执行的结果」；任务实例的
-- 运行时状态（进行中/取消/进度）属任务执行中心，本表不承载。
-- partial = 上一轮未跑完（人工经任务中心取消实例，已完成部分保留，下轮从断点续接；
--           区别于 skipped「整轮未执行」与 failed「执行出错」）。
...
CREATE TABLE IF NOT EXISTS {table} (
    ...
    last_status  VARCHAR(16) NOT NULL DEFAULT '',   -- success / failed / skipped / partial
...
COMMENT ON COLUMN {table}.last_status IS 'success / failed / skipped / partial';
