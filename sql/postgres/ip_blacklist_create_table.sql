...
CREATE TABLE IF NOT EXISTS {table} (
    ...
    block_type  SMALLINT NOT NULL DEFAULT 1,      -- 拉黑原因类别（1-10，含义见表头注释）
...
COMMENT ON TABLE {table} IS 'IP 黑名单表：动态 IP 黑名单（管理面录入/批量导入），与外挂 rules/ip_blacklist.txt 取并集；热路径只读内存快照；block_type 1-10 枚举见列注释';
...
COMMENT ON COLUMN {table}.block_type IS '拉黑原因类别 1-10：1=IP黑名单 2=限流(429) 3=方法不允许 4=请求体超限(413) 5=风险路径 6=路径遍历 7=SQL注入 8=XSS 9=爬虫/扫描器UA 10=路径/UA规则deny；仅管理面过滤统计，非运行时匹配依据；枚举定义见 plugins/shield/block_type.go';
