...
-- 现收敛为 access_log 1 次扫描 + shield_event 1 次扫描 + 分位数在物化集上聚合），口径与旧版逐项一致。
-- 参数顺序：from($1,$3), to($2,$4)：CTE a（access_log 范围行）与 CTE s（shield_event 聚合）各一对。
...
--                范围无行时 SUM 为 NULL，COALESCE 归 0（与旧版 COUNT 口径一致）。
...
--   延迟口径：total_ms 的 AVG / PERCENTILE_CONT 连续分位数（分位数聚合基于物化的 CTE a，
--                METRICS_WINDOW 拆分项，随所选范围精确统计）；无行时为 NULL。
WITH a AS (
  SELECT client_ip, user_agent, status_code, total_ms,
         LOWER(SPLIT_PART(path, '?', 1)) AS ppath
  FROM {table}
  ...
  SELECT COUNT(*) AS req_ok,
         ...
         COUNT(DISTINCT ROW(client_ip, COALESCE(user_agent, ''))) AS uv,
  ...
  SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY total_ms) AS lat_p50,
         PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY total_ms) AS lat_p95,
         PERCENTILE_CONT(0.99) WITHIN GROUP (ORDER BY total_ms) AS lat_p99
  FROM a
)
