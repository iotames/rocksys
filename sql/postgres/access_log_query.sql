...
WHERE a.time >= $1 AND a.time <= $2
  ...
  AND ($9 = '' OR SUBSTR(CAST(a.status_code AS TEXT), 1, 1) = $10)
  AND ($11 = 0 OR a.status_code >= 400)
ORDER BY
  -- $12/$13 经 lib/pq 以未知类型下发，PG 对 CASE <param> WHEN <int> 的消解会判为 text
  -- （text = integer 报错），显式 CAST 固化为整数比较（sqlite/mysql 无此问题）。
  CASE CAST($12 AS INTEGER) WHEN 1 THEN a.total_ms WHEN 3 THEN a.egress_ms ELSE -1 END DESC,
  CASE CAST($13 AS INTEGER) WHEN 2 THEN a.total_ms WHEN 4 THEN a.egress_ms ELSE -1 END ASC,
  a.id DESC
LIMIT $14 OFFSET $15
