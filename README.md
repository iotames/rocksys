...
开源的流量网关，部署在业务系统与外部访问之间，统一承接安全防护、访问控制与流量调度。**业务系统向前迭代，磐石在后面保持稳定。**

一个实例，即具备以往需要"防火墙 + 网关 + 负载均衡 + 审计系统"多套组件拼出的入口能力：

- **安全防护**：攻击识别、自动封禁"小黑屋"、爬虫识别、黑白名单、限流；
- **流量统计**：PV/UV/独立 IP、拦截与攻击源趋势报表、GeoIP 地理位置（世界/中国热力地图 + 访问/总拦截 Top 地区排名联动；GeoLite2 mmdb 可选，缺失自动降级；功能总开关 `GEOIP_ENABLED` 可一键停用，默认开启）；
- **路由分发**：数据库化三层路由（规则 → 负载均衡器 → 节点），加权轮询 / 最小连接两种策略、会话保持（sticky Cookie）、节点健康检查与故障自动摘除、主备容灾；
- **可视化管控**：管理控制台一屏尽览，规则改动 3 秒热生效；
- **轻量交付**：单文件部署，三平台发布。

演进路线三部曲：**可靠**（精细化限流、过载保护）→ **智能**（熔断、故障自愈、多线路调度）→ **生态**（组件生态、托管服务、AI 流量治理），详见 [docs/ROADMAP.md](docs/ROADMAP.md)。

> 核心哲学：**只有转发是必须的**。其余一切皆是可选增强，默认全关，可随时热插拔；任何组件掉链子，自动降级并实时告警，可用性优先、绝不无声失效。

磐石的地基由三个自研基础库构成，各自独立开源：

- [easyserver](https://github.com/iotames/easyserver)：简单易用的 HTTP/TCP 服务器运行时，磐石的高性能底座；
- [easyconf](https://github.com/iotames/easyconf)：极简配置工具，命令行 > 环境变量 > 配置文件三级取值，全量配置集中管理；
- [easydb](https://github.com/iotames/easydb)：通用数据库接口，一套 API 操作 MySQL / PostgreSQL / SQLite 等多种数据库。
...
开启组件开关 `DISPATCH_ENABLED` 后，网关具备三层路由能力：**路由规则 → 负载均衡器 → 上游节点**（数据存数据库路由四表，不经配置文件管理）。

- **规则匹配**：域名（留空 = 任意；非空精确匹配）× 路径（前缀 / 精确 / 模式三种类型），按序号升序命中即停；未命中走 `--upstream` 默认后端；
- **负载均衡**：`round_robin` 平滑加权轮询 / `least_conn` 最小连接优先；可选会话保持（网关自种 Cookie 粘性直路由）；节点分高优 / 备份两级（高优全不健康才启用备份）；
- **健康检查**：节点登记探活路径后周期探测，故障自动摘除、恢复自动回归；
- **管理**：经 WebUI「组件 → 分发 dispatch」详情页「路由规则 / 负载均衡器 / 上游节点」页签或管理接口 `/admin/dispatch/*` 维护（节点登记、均衡器编排、规则管理、命中测试），保存即热更免重启；DB 不可用时自动降级为全部流量走默认后端，恢复后自动重建。

详见 `docs/CONFIGURATION.md`（配置项）与 `docs/api/dispatch.md`（接口契约）。
...
| 防护误拦 | 关闭 shield 或调整规则；搜索引擎爬虫被 UA 拦截误伤时，将爬虫 UA 子串加入 `hotscripts/rules/ua_whitelist.txt`（白名单优先、≤3s 热更，仅豁免爬虫 UA 拦截一步） |
...
- **换库与数据迁移**：管理台「数据库 → 表数据」页签可视化完成——配置外部数据源（三方言）、对目标库对齐表结构、按批迁移数据（同名字段直迁、表级进度、可取消、迁完自动重置自增序列）；长任务统一走任务执行中心后台执行，GeoIP 数据同步与 SQL 手工执行同样不再受 HTTP 超时限制。
