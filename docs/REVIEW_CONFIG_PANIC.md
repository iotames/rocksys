...
装配期注册（cmd/rocksys/main.go，截至脚本入口收敛时点的装配期注册快照）：`HOT_SCRIPTS_DIR`、`TRUSTED_PROXIES_FILE`、`SCRIPT_TIMEOUT`、`DB_DRIVER`、`DB_DSN`、`MQ_ENABLED`、`MQ_POLL_INTERVAL`、`MQ_MAX_RETRIES`、`MQ_BASE_BACKOFF`、`MQ_CONSUMER_BASE_URL`、`REGISTRY_ADDR`、`REGISTRY_TTL`、`REGISTRY_STATIC_FILE`、`OBJECT_BASE_DIR`。
