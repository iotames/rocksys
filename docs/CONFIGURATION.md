...
| `GEOIP_SYNC_INTERVAL` | `60` | GeoIP 关联表（`geoip_list`）自动同步间隔，**int 分钟**（间隔内置开关语义，不设独立开关）：**0=关闭**自动同步（手动同步不受影响）；有效最小 **10**（防设置过小耗尽资源），<10（非 0）或非法回落 60。**生效前置 = mmdb 已加载**（未加载时定时器不启动，配置任意值均不生效；放置文件后须重启）；运行中修改**下一轮生效**。登记与状态见 WebUI「配置 → 定时任务」页。**执行模型**：到点经任务中心提交同步实例（与手动同步同一执行体），受互斥公共集管控——同集有任务在跑（人工长任务/上一轮未完）时该轮**跳过**（定时任务页记「跳过」，下轮到点续接，游标增量无数据丢失）；同步无时间预算（后台任务默认无超时） |
...
#   生效前置 = mmdb 已加载（未加载时定时器不启动）；运行中修改下一轮生效
