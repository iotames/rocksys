...
| `GEOIP_ENABLED` | `true` | **GeoIP 地理位置功能总开关**（bool，默认开启）。开关语义内聚在 `internal/geoip` 解析器单点裁决（`Enabled`/`Ready`/`Lookup` 少数入口），消费方不各自读配置。**关闭后**：读侧实时地区解析停用（明细/Top IP 地区列显示「服务未开启」；JOIN 命中的 geoip_list 历史地区照常显示）、**自动同步定时器停摆**（每轮 tick 就绪复查，运行期关闭 ≤1 分钟内停摆）、定时任务登记降级、概览页地理位置卡降级为「功能未开启」引导。**例外——手动同步不受本开关限制**：定位为特殊场景的异步 DB 维护任务（不影响主程序转发），仅要求 mmdb 已加载（数据库页入口照常可用）。**支持热更**（秒级生效）；已知边界：启动时禁用、运行期再开启，自动同步定时器需重启拉起（手动同步即时可用） |
...
| `GEOIP_SYNC_INTERVAL` | `60` | GeoIP 关联表（`geoip_list`）自动同步间隔，**int 分钟**（间隔内置开关语义，不设独立开关）：**0=关闭**自动同步（手动同步不受影响）；有效最小 **10**（防设置过小耗尽资源），<10（非 0）或非法回落 60。**生效前置 = GeoIP 服务就绪（`GEOIP_ENABLED=true` 且 mmdb 已加载）**（未就绪时定时器不启动）；运行中修改**下一轮生效**。登记与状态见 WebUI「配置 → 定时任务」页。**执行模型**：到点经任务中心提交同步实例（与手动同步同一执行体），受互斥公共集管控——同集有任务在跑（人工长任务/上一轮未完）时该轮**跳过**（定时任务页记「跳过」，下轮到点续接，游标增量无数据丢失）；同步无时间预算（后台任务默认无超时） |
| `GEOIP_SYNC_IPS_PER_RUN` | `50000` | GeoIP 同步**单趟处理的缺失 IP 上限**，**int 个**：**0=不限**（一趟扫到表尾为止）；负数回落默认。与 `GEOIP_SYNC_SCAN_CAP` 两上限先到为准，配额用完即收工、下轮断点自动续扫；**流式回写**（每万行块边扫边写，内存恒定，任务页实时进度）。增量稳态每趟新 IP 远小于配额（行为不变）；存量回填调大可减少趟数，极端情况可临时调 0 后恢复默认。运行中修改**下一趟生效** |
| `GEOIP_SYNC_SCAN_CAP` | `1000000` | GeoIP 同步**单趟扫描日志行数上限**，**int 行**：**0=不限**；负数回落默认。与 `GEOIP_SYNC_IPS_PER_RUN` 先到为准——即使缺失行极稀疏也保证单趟耗时可控（SQLite 写锁窗口有界）；运行中修改**下一趟生效** |
| `CONF_DIR` | `conf` | 全局配置目录（相对工作目录，开发规范下即 `bin/conf/`）：外部数据源文件 `dsn.json` 存放于此（JSON 明文，与 `bin/.env` 同级安全边界，不入库不入 Git）。每次读写**实时取当前生效值**拼接路径（改配置即换位置）；文件懒创建——首次写入自动建目录与文件，读取时不存在视为空配置。**变更后原位置文件不自动搬迁**（需手工移动，否则按新位置视为空配置） |
...
# ===== GeoIP（internal/geoip + geoip_list 关联表同步，GEOIP_LIST 方案）=====
# 功能总开关（默认开启；关闭后地区实时解析与自动同步全部停用，明细地区列显示「服务未开启」，
# 概览页地理卡降级未开启引导；数据库页手动同步不受本开关限制——特殊异步 DB 维护任务）；
# 热更秒级生效（启动时禁用的自动同步定时器需重启拉起）
GEOIP_ENABLED = true
...
# 外部数据源目录（数据迁移用）：<CONF_DIR>/dsn.json 存外部数据源配置（JSON 明文，与 .env 同级安全边界，
# 不入库不入 Git）；每次读写实时取当前生效值拼路径，变更后原位置文件不自动搬迁（需手工移动）
CONF_DIR = conf

# 自动同步间隔（分钟）：geoip_list 增量构建的唯一控制项（不设独立开关）
#   0 = 关闭自动同步（手动同步不受影响）；最小 10（更小/非法回落 60）；默认 60（=1 小时）
#   生效前置 = GeoIP 服务就绪（GEOIP_ENABLED=true 且 mmdb 已加载）；运行中修改下一轮生效
GEOIP_SYNC_INTERVAL = 60

# 同步单趟配额（两上限先到为准；0=不限一趟扫到表尾；负数回落默认；运行中修改下一趟生效）
#   增量稳态每趟新 IP 远小于配额（行为不变）；存量回填调大可减少趟数，下轮断点自动续扫
GEOIP_SYNC_IPS_PER_RUN = 50000
GEOIP_SYNC_SCAN_CAP = 1000000
...
> 每个挂件默认关闭；`bin/.env` 里写 `XXX_ENABLED=true` 即等于启用（重启后按配置自动挂载，热改即时联动）；也可在 WebUI「组件」页或 `rockctl switch on` 开启（会自动持久化回 `.env`）。布尔值仅识别 `true`/`false`（大小写不敏感），非法值按 `false` 处理并在启动时告警。
