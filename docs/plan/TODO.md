...
| 2026-09-22 | ZCode 会话 | 验收反馈 | 修复 6 项（弹层误关/URL 路径拒绝/警告常驻/标签链路/菜单并入组件详情页签），全量测试绿、浏览器实看通过 |
| 2026-09-22 | ZCode 会话 | 验收反馈2 | 规则标签筛选落地（三方言脚本+前端筛选栏）；通知组件 API 分层重构（notify 语义层收敛参数 + toastRaw 底层逃生口，全站 193 处迁移，移除 Rock.ui.toast 导出）；修复 PG tag/关键词传参个数 |
| 2026-09-22 | ZCode 会话 | 验收反馈3 | dispatch 详情页签重构：状态/路由规则/负载均衡器/上游节点/配置（配置统一最后入规范）；描述整合、规模卡迁状态页签（可点击直达）；geoip_list 丢表排查结论：全仓运行时代码无删表路径，暂搁置 |
