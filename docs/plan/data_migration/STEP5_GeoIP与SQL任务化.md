...
## 实施步骤（完成一项立即勾选保存）
- [ ] adminapi 暴露 SubmitTask 帮助方法（供 main.go 装配处提交任务）
- [ ] geoip_sync 任务化（进度=表计数快照、Result=报告文本）
- [ ] exec background 分支（任务内复用既有逐条执行与审计逻辑）
- [ ] 单测：exec background 提交→轮询拿到逐条结果；互斥拒绝文案

## 验证
- 接手核实命令：`go test ./internal/adminapi/ -run TestDBExec && go build ./...`
- 本步完整验证：
  - [ ] `go build -tags dev -o bin/rocksys ./cmd/rocksys && go vet ./... && go test ./internal/adminapi/`
