...
## 实施步骤（完成一项立即勾选保存）
- [ ] openTargetDB(code) 帮助函数：按 code 从 dsn 存储取数据源 → db.Open(driver, dsn)（不传 hub）
- [ ] handleMigrateSchema：diff 预览（items + DDL 文本）
- [ ] handleMigrateSchemaApply：向任务中心 Submit（CreatedBy=schema_apply），Run 内拆句逐条执行、遇错停，结果 JSON 归 Progress/Result
- [ ] 单测：sqlite 目标库 diff 空态与建表后零 diff；apply 任务终态 done 且目标表存在；重复提交被互斥拒绝
