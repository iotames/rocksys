...
> 状态：**已实施并归档** · B1-B5 全部完成（含文档同步与真库验收），细节以验收记录（§6 变更记录）为准。
> 归档说明：2026-08-30 功能开发与验收全部完成后，本文档移入 `docs/done/`；分步执行文件（原 `docs/plan/db_schema_sync/STEP1..STEP5` 与 `docs/plan/TODO.md`）随验收完成删除（进度已由 §6 变更记录收口），实施方法论沉淀见 `docs/plan/README.md`。
> 最后更新：2026-08-30
...
| # | 决策点 | 结论 |
|---|--------|------|
| 1 | 差异自动生成 SQL 的范围 | **保守：安全项才生成**——缺表（建表原文+配套索引）+ 缺普通列（ADD COLUMN）+ 缺索引（仅缺失项单条语句）自动生成；缺 PK/UNIQUE/自增列标注"需人工"（SQLite 不可 ADD，不生成）；类型/非空/默认值不一致与库中多余对象**仅提示不生成**（SQLite 改列需重建表、DROP 危险，跨方言不可靠） |
...
| 级别 | 差异类型 | 处理 | 生成内容 |
|---|---|---|---|
| A | 缺表 | 自动 | 建表脚本原文 + 配套索引脚本（`{table}` 替换后，多语句由执行器拆条；附带索引使新库一轮同步即闭环，与组件 EnsureTable「建表+索引」同语义） |
...
| D | 缺索引 | 自动 | 仅缺失索引的单条 CREATE INDEX（不整份重放——执行器遇错即停，重放会在已有索引处报 Duplicate key 导致剩余索引永远补不齐） |
...
- `POST /admin/db/exec`：body `{sql}`；服务端**拆句器**（分号切分，感知字符串字面量与注释内的分号）逐条执行，**遇错即停**（DDL 无跨方言统一事务语义，返回已执行到的位置），响应 `{results:[{sql, ok, rows, error}], executed, failed}`；进程内互斥（已有执行在途时回 409，防并发 DDL 交叉）、请求体上限 1MB、执行绑定请求上下文（服务停机可取消）。
...
实施曾逐一拆分为 `docs/plan/db_schema_sync/STEP1..STEP5`（B1-B5）并按 `docs/plan/TODO.md` 总纲顺序执行，**已全部完成并验收**；STEP 文档与总纲随归档删除，执行过程结论已合并进 §6 变更记录。
...
- 2026-08-30 已知边界（验收发现）：存量表「列存在但约束缺失」（如表级 UNIQUE 被人删除）不在自动检测范围——C 级仅覆盖"缺约束列"，约束比对需扩展 catalog 查询（索引唯一性），本期不做；操作手册路径：缺约束属人工项，可经「执行SQL」手写补齐。
- 2026-08-30 验收修订（真库验收发现）：A 级生成附带配套索引（原仅建表原文，新库一轮同步后仍缺全部索引需二次检查）；D 级改为仅生成缺失索引的单条语句（原整份重放与「遇错即停」矛盾）；C 级判定补齐表级 PRIMARY KEY/UNIQUE 约束列（mysql 表级 `UNIQUE KEY uk(col)` 不产出 ColumnDef，此前误判 B 级自动补列会静默丢失约束）；pg catalog 自增识别改用 `pg_get_expr(adbin,…)`（`adbin` 为 pg_node_tree，`LIKE`/`::text` 均不可靠）；exec 端点补并发互斥（409）/请求体上限/上下文取消。验收（真服务 + 真库 mysql/pg：全新库 7 表 A 级一轮闭环、单索引缺失 D 级精确补齐、catalog 自增/默认值/可空识别、缺表→执行→零差异）全部通过。
