...
> 状态：已确认（2026-08-31 用户明示「实施本计划」开工；总纲 `docs/plan/TODO.md`，步骤文档 `docs/plan/ua_whitelist/`）。
...
| D6 | 黑名单文件是否改名 | **不改名**，继续用 `crawler_ua.txt` | 用户已部署的外挂覆写文件不失效；页面显示名改为「UA黑名单」即可 |
...
| D8 | WebUI 结构 | 「黑白名单」页签子页签由 2 个改 4 个：IP黑名单 / IP白名单 / UA黑名单 / UA白名单。IP 两个沿用现有 DB CRUD 视图；UA 两个做**简单查看 + 追加**（无 DB 表，读写规则文件三端点），页内提示"完整编辑请前往「文件编辑」页签"并带跳转链接；「文件编辑」页签保留不动 | 用户拍板；文件编辑页签是全量规则文件统一入口，保留 |
...
| D10 | UA 名单页形态升级 | 「黑白名单」页签的 UA 黑/白名单页升级为**完整行级管理**：生效模式表格 + 单条删除（确认弹层，仅移除模式行、注释行保留）+ 追加 + 恢复默认（回退内嵌默认内容）；「文件编辑」页签的整文编辑**保留不动**（两处粒度不同、不冲突，D8 维持） | 用户拍板（验收反馈）：借鉴 IP 白名单单页自包含管理的形态，UA 页不需要整文编辑能力 |
...
- 新增内嵌规则文件 `plugins/shield/rules/ua_whitelist.txt`，内容按 D7：
  ...
  ```
  # UA白名单：每行一个模式，匹配时转小写（子串匹配）。
  # 优先级：白名单 > 黑名单；UA 黑名单开关（SHIELD_WAF_CRAWLER_UA）开启后，
  # 命中白名单的 UA 在黑名单判定前放行、仅豁免「UA黑名单」这一步，
...
- `plugins/shield/rules_admin.go`：`ruleFileMetas` 增加 `{ruleFileUAWhitelist, "UA白名单", "无开关，有数据即生效；UA 黑名单开关（SHIELD_WAF_CRAWLER_UA）开启后命中即在黑名单判定前放行，仅豁免该步，其余检测照常"}`；`crawler_ua.txt` 的 title 由「爬虫 UA 特征」改为「UA黑名单」（desc 点明其开关即 `SHIELD_WAF_CRAWLER_UA`、白名单优先）。文件编辑页签白名单由 6 个变 7 个。相关代码注释（配置项 title、waf.go 判定处）统一称谓：`SHIELD_WAF_CRAWLER_UA`＝UA黑名单开关。
...
- 2026-08-31：追加 D9（用户拍板）：UA 白名单不设开关，同 IP 白名单一样有数据即生效；`SHIELD_WAF_CRAWLER_UA` 正名为「UA黑名单开关」，代码注释统一称谓；体验规范新增「开关设置原则」条款（docs/webui.md）——开关取舍以用户体验与流程设计的具体场景为准，不多设也不少设。
- 2026-08-31：验收反馈追加 D10（用户拍板）：UA 名单页升级为完整行级管理（生效模式表格 + 单条删除 + 追加 + 恢复默认），文件编辑页签整文编辑保留（两处粒度不同、不冲突）；ualist.js 相应重写，rules_admin.go 的 crawler_ua/ua_whitelist desc 注明「黑白名单页签行级管理、本页整文编辑」。实施中修复两个前端缺陷：① save() 成功路径 state.saving 未复位，首次操作后所有后续操作被静默拦截；② 保存后立即重拉会读到 ScriptHub ≤3s 热更前的旧缓存打回旧状态，改为乐观渲染 + 3.5s 后延迟对账。浏览器实操：追加/删除/恢复默认即时反馈与落盘全过。
- 2026-08-31：用户拍板改名：展示称谓「UA(爬虫)黑名单/白名单」统一简化为「UA黑名单/UA白名单」（子页签、卡片标题、ruleFileMetas title/desc、配置项 title、规则文件头注释、代码注释全链路；文件名 crawler_ua.txt/ua_whitelist.txt 与 SHIELD_WAF_CRAWLER_UA 不变）。全仓 grep 无残留，测试+vet 绿，浏览器实操页签/标题生效。
- 2026-08-31：终验闭环——全量 go test（0 失败）+ go vet + 生产构建全绿；浏览器实操（四子页签、行级管理追加/删除/恢复默认闭环、文件编辑页签可见新文件、跳转链接）与 curl 语义验证（§4.4 五场景）全过。文档同步：docs/webui.md 新增 §4.13（UA黑/白名单行级管理）与 §4.14（开关设置原则）、§6 功能清单更新为 7 个规则文件；README 故障处理表补 UA 白名单误拦自救指引；Makefile 注释 5→7；数据字典确认零变更。已知边界：shield 配置项（含 SHIELD_WAF_CRAWLER_UA）改动需重启生效，属既有行为本期不扩大整改面。项目转「待人类验收」。
- 2026-08-31：归档前回填区核对（逐 STEP 与本变更记录比对）——STEP1 改名偏差与「配置改动需重启」已知边界、STEP2 两缺陷修复均已在此有载；补录 STEP3 执行过程违规：STEP1/2 完成与改名偏差曾漏同步总表状态列/STEP 状态行/锚点清单（违反铁律1 收口三同步），经用户指出后逐项补正，权威状态以总纲为准。核对结论：无未并入偏差、无未处理临时决策（本项目管理期未产生 <项目>_DECISIONS.md）。
