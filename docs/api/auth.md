...
| `store_ready` | bool | 用户存储是否就绪（数据库不可用/建表失败时为 false） |
| `store_error` | string | 存储降级原因（就绪时为空；如「数据库初始化失败（driver=postgres）：…」） |

**前端引导逻辑：** `auth_required=false` → 直接进入控制台；`has_user=false` 且 `store_ready=false` → 数据层降级引导页（展示 `store_error` 与排查步骤，注册/登录均被后端 503 拦截）；`has_user=false` → 注册页；`setup_mode=true` → 重置页；否则 → 有 token 进控制台，无 token 显示登录页。
...
**失败 `503`：** 用户存储未就绪（数据库不可用/建表失败），错误信息含降级原因与排查引导（检查数据库配置，修复重启后重试）。
...
**失败 `503`：** 用户存储未就绪（数据库不可用），错误信息含降级原因与排查引导。
