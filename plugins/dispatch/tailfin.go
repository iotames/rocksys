// Tail 收尾件（S4 least_conn 在途递减收尾）：转发完成（响应返回）后按
...
//   - 递减按节点 id：选点阶段由主件（STEP5）把选中节点 id 写入 DataFlow
...
// （rocksys: 前缀惯例）。主件（STEP5）选点成功后写入，收尾件读取后递减。
...
// NewTailFin 创建收尾件（独立实例化，装配归 STEP5）。
